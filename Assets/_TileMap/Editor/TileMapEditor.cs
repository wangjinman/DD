...
public class TileMapEditor : Editor {
	...
	public override void OnInspectorGUI(){
		...
		Columns = EditorGUILayout.IntField("Colums",map.Columns);
		Rows = EditorGUILayout.IntField("Rows",map.Rows);
		spriteTmp = (Sprite)EditorGUILayout.ObjectField(spriteTmp, typeof(Sprite),false);
		map.Columns = Columns;
		map.Rows = Rows;
		EditorGUILayout.BeginVertical();
		var opt = new String[] {"Gen Box Collider For Each","Gen Box Collider"};
		gbOpt = EditorGUILayout.Popup(gbOpt,opt);
		EditorGUILayout.EndVertical();
		if(GUILayout.Button("Gen Box Collider")){
			if (gbOpt == 0){
				GenBoxColliderForEach();
			} else {
				GenBoxCollider();
			}
		}
		if (GUILayout.Button ("Modify Tile Name")) {
			ModifyName();
		}
	}
	...
	private void Draw()
	{
		...
		var cube = GameObject.Find(string.Format("{0}_Tile_{1}_{2}", map.name,tilePos.x, tilePos.y));
		...
		SpriteRenderer sp = cube.AddComponent<SpriteRenderer>();
		sp.sprite = spriteTmp;
		
		// give the cube a name that represents it's location within the tile map
		cube.name = string.Format("{0}_Tile_{1}_{2}", map.name,tilePos.x, tilePos.y);
	}
	...
	void GenBoxCollider(){
		var map = (TileMap)this.target;
		map.gameObject.AddComponent<Rigidbody2D>();
		BoxCollider2D bc = map.gameObject.AddComponent<BoxCollider2D>();
		bc.size = new Vector2(map.TileWidth * map.Columns,map.TileHeight * map.Rows);
		float x = map.TileWidth * map.Columns / 2.0f;
		float y = map.TileHeight * map.Rows / 2.0f;
		bc.center = new Vector2(x,y);
	}

	void GenBoxColliderForEach(){
		var map = (TileMap)this.target;
		ArrayList list = map.GetTiles();
		foreach (GameObject obj in list){
			obj.AddComponent<Rigidbody2D>();
			BoxCollider2D bc = obj.gameObject.AddComponent<BoxCollider2D>();
			bc.size = new Vector2(map.TileWidth,map.TileHeight);
		}
	}
	...
	void ModifyName(){
		...
		foreach (GameObject obj in list){
			...
			char[] seperator = {'_'};
			string[] l = name.Split(seperator);
			string newName = name.Replace(l[0],map.name);
			obj.name = newName;
		}
	}
}
